...
var SCRIPT_VERSION = "2.04"; // Date: 2025-04-30
...
const _helpText =  
'''This InDesign script is an aid for quickly exporting page ranges to PDFs with labels and versioning. It is particularly usefull in editorial workflows in which you repeatedly export new versions of pages (i.e. of edited articles) for your editors or customers.
...
function exportPDFs(validatedInputs) {
        ...
        if (match) {
            ...
            if (DEBUG_MAC && $.os.indexOf("Mac") > -1) {
                try {
                    ...
                    logFile.open('a');
                    logFile.writeln("\n--- Path Debug " + new Date().toISOString() + " ---");
                    logFile.writeln("Original path: " + _path);
                    logFile.writeln("Original filename: " + fileName);
                }
                catch(le) {
                    $.writeln("Warning: Could not write path debug to log: " + le);
                }
            }
            ...
            if (DEBUG_MAC && $.os.indexOf("Mac") > -1) {
                try {
                    logFile.writeln("Final filepath: " + filePath);
                    logFile.close();
                }
                catch(le) {
                    $.writeln("Warning: Could not write final path to debug log: " + le);
                }
            }
    ...
    function exportSinglePDF(exportInfo) {
        try {
            ...
            // Prüfe Schreibrechte
            var testFile = new File(exportFolder + "/.write_test");
            ...
            if (DEBUG_MAC && $.os.indexOf("Mac") > -1) {
                try {
                    ...
                    logFile.writeln("Parent folder exists: " + File(exportInfo.filePath).parent.exists);
                    logFile.close();
                } catch(le) {
...
function getFormattedPath(inputPath, basePath) {
    ...
    try {
        if ($.os.indexOf("Mac") > -1) {
            ...
            // Windows branch – keep it simple
            if (/^[a-zA-Z]:/.test(inputPath)) {
                return new Folder(inputPath.replace(/[\/\\]+$/, '')); // strip trailing slash
            } else {
...
function createSafePath(basePath, fileName) {
    if ($.os.indexOf("Mac") > -1) {
        ...
        return decodeURI(tempFile.fsName);
    }
    return new File(new Folder(basePath).fsName + '/' + fileName).fsName;
}
...
function validateFolderPath() {
    try {
        if (d.isValid && d.saved && d.fullName !== null) {
            ...
            var formattedFolder = getFormattedPath(inputPath, d.fullName.path);

            if (!formattedFolder.exists) {
                var parentFolder = new Folder(formattedFolder.parent);
                if (!parentFolder.exists) {
                    alert(__("The specified path does not exist and cannot be created:") + " " + formattedFolder.fsName);
                    return false;
                }
                
                var testFolder = new Folder(formattedFolder.fsName + "/.test_" + Math.round(Math.random() * 1000));
                var canCreate = testFolder.create();
                if (canCreate) {
                    testFolder.remove();
                } else {
                ...
                // Teste Schreibrechte im existierenden Ordner
                var testFile = new File(formattedFolder.fsName + "/.write_test");
                var canWrite = testFile.open("w");
                if (canWrite) {
                    testFile.close();
                    testFile.remove();
        ...
        alert(__("Path validation error:") + " " + e.message);
        return false;
    }
}