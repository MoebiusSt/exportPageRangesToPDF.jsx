...
var SCRIPT_VERSION = "2.11"; // Date: 2026-06-30
// v2.11: Fix getFileVersion() so versioning works when a label/identifier contains
//        characters that sanitizeFileNameSegment() rewrites (esp. "/" and "\", which a
//        glob mask reads as path separators). Previously such labels never matched
//        existing files, so every run reported v1 and overwrote the existing _v1 file.
...
/**
 * Determines the next version number for a given identifier (label or page range).
 *
 * Versioning is keyed on the identifier, not on the page range: the same label across
 * different page ranges shares one version counter. The ".*" in the pattern below is the
 * equivalent of the previous glob "*" and wildcards the page-range part of the file name.
 *
 * IMPORTANT (bug fix v2.11): the identifier is now sanitized with sanitizeFileNameSegment()
 * so it matches the on-disk file names, which are themselves built from sanitized segments.
 * The previous implementation fed the RAW identifier into Folder.getFiles(maskString).
 * In a file-system glob mask, "/" and "\" are interpreted as PATH SEPARATORS, so a label
 * containing slashes (e.g. "... // ... NM/MT") turned the mask into a non-existent sub-path
 * and matched nothing. As a result the highest existing version was never found and every
 * run returned v1, silently overwriting the existing _v1 file. The same failure occurred for
 * any character that sanitizeFileNameSegment() rewrites ( \ / : * ? " < > | ), because the
 * raw identifier no longer equals the sanitized form stored on disk.
 *
 * @param {String} path       Export folder path
 * @param {String} baseName   Document base name without extension
 * @param {String} identifier Raw label or page range (sanitized inside this function)
 * @returns {Number} Next version number (highest found + 1, or 1 if none)
 */
function getFileVersion(path, baseName, identifier) {
    try {
        // Sanitize exactly as the export builds file names, so the match succeeds.
        var safeIdentifier = sanitizeFileNameSegment(identifier);

        // Match "<baseName> ... _<identifier>_v<digits>.pdf" via a RegExp callback instead
        // of a glob mask. This treats "/", "\" and other special chars literally rather than
        // as path separators / wildcards. The leading "_" before the identifier preserves the
        // original semantics and prevents partial-label collisions (e.g. "X" vs "AX").
        var versionRegex = new RegExp(
            "^" + escapeRegExp(baseName) + ".*_" +
            escapeRegExp(safeIdentifier) + "_v(\\d+)\\.pdf$",
            "i"
        );

        // Safe filename decode: a stray "%" in a name must not throw and abort detection.
        ...
        var files = Folder(path).getFiles(function (file) {
            return (file instanceof File) && versionRegex.test(decodeName(file));
        });

        var highestVersion = 0;
        if (files) {
            for (var i = 0; i < files.length; i++) {
                var match = decodeName(files[i]).match(/_v(\d+)\.pdf$/i);
                if (match) {
        ...
        Logger.debug("Found highest version " + (highestVersion + 1) + " for identifier '" + safeIdentifier + "'");
...
function decodeURIComponentSafe(str) {
    // Normalize hex digits to uppercase so the regex below works uniformly
    var normalized = str.replace(/%([0-9A-Fa-f]{2})/g, function(m, h) {
        return '%' + h.toUpperCase();
    });
    // Decode runs of consecutive %XX sequences as UTF-8
    return normalized.replace(/(?:%[0-9A-F]{2})+/g, function(seq) {
        var bytes = [];
        var i = 0;
        while (i < seq.length) {
            if (seq.charAt(i) === '%') {
        ...
        while (j < bytes.length) {
            ...
            if (b0 < 0x80) {
                ...
                result += String.fromCharCode(((b0 & 0x1F) << 6) | (bytes[j + 1] & 0x3F));
                j += 2;
            } else if ((b0 & 0xF0) === 0xE0 && j + 2 < bytes.length) {
                result += String.fromCharCode(((b0 & 0x0F) << 12) | ((bytes[j + 1] & 0x3F) << 6) | (bytes[j + 2] & 0x3F));
                j += 3;
            } else if ((b0 & 0xF8) === 0xF0 && j + 3 < bytes.length) {
                // 4-byte sequence (above U+FFFF) — emit as surrogate pair
