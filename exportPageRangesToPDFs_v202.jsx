...
var SCRIPT_VERSION = "2.02"; // Date: 2025-02-08

if (typeof DEBUG_MAC === 'undefined') {
    var DEBUG_MAC = true;
}

if (typeof _helpText === 'undefined') {
const _helpText =  
'''This InDesign script is an aid for exporting page ranges to PDFs with labels and versioning. It is particularly usefull in editorial workflows in which you repeatedly export new versions of pages (i.e. of edited articles) for your editors or customers.

Features:
- Export multiple page ranges to individual PDFs and stores your page range entries for re-use. See EXAMPLE 1.
- Specify the export-folder and PDF-preset in the settings. It is stored with the document.
- Specify a identifier for each range. (i.e. to name an article). See EXAMPLE 2.
...
[DocumentName]_pages_[PageRange]_[Label-from-brackets]_v[autoVersion].pdf
...
EXAMPLE 1:
Enter one page or page-range per line. Don't use commas. Page Ranges are allowed to overlap. They don't need to be sorted. Enter:
1
5-7
16
5-20
...
EXAMPLE 2:
Specify a label/identifier for a page-range in brackets (optional). It retrieves:
...
If you don't want to increment the version, to overwrite the last pdf file, add a minus "-" to the end of a line. It will overwrite those PDFs during export and will not create a next version pdf. Enter:
...
EXAMPLE 4:
Click [-GET FROM DIRECTORY-] to retrieve a list of page ranges from your former exports. The script scans the directory for files matching the naming schema and generates a list. If it finds "MyDocument_pages_1_Introduction_v1.pdf" and "MyDocument_pages_16_v2.pdf" and "MyDocument_pages_21-24_The Doors_v1.pdf" it retrieves:
...
function getFormattedPath(inputPath, basePath) {
    ...
    try {
        if ($.os.indexOf("Mac") > -1) {
            // Mac Debug-Logging initialisieren
            if (DEBUG_MAC) {
                try {
                    logFile = new File(Folder.desktop + "/exportPDF_debug.txt");
                    logFile.open('a');  // Append-Modus
                    logFile.writeln("\n--- Debug Log " + new Date().toISOString() + " ---");
                    logFile.writeln("Input Path: " + inputPath);
                    logFile.writeln("Base Path: " + basePath);
                    logFile.writeln("OS: " + $.os);
                    
                    // POSIX-Pfad-Check
                    var usePosixPaths = (function() {
                        try {
                            return File(app.filePath + "/Resources/Cookies/ENABLE_POSIX_PATH").exists;
                        } catch(e) {
                            return false;
                        }
                    })();
                    logFile.writeln("PosixPath: " + usePosixPaths);
                } catch(e) {
                    $.writeln("Warning: Could not create debug log: " + e);
                }
            }
            ...
            if (DEBUG_MAC && logFile && logFile.exists) {
                logFile.writeln("Normalized Path: " + path);
            }
            var result = new Folder(path);
            if (DEBUG_MAC && logFile && logFile.exists) {
                logFile.writeln("Final Folder Path: " + result.fsName);
                logFile.writeln("Folder exists: " + result.exists);
            }
            return result;
            
        } else {
            // Windows-Pfadbehandlung bleibt unverändert
        ...
        $.writeln("Error in path handling: " + e.message);
        if (DEBUG_MAC && logFile && logFile.exists) {
            logFile.writeln("Error occurred: " + e.message);
        }
        
        // Fallback: Verwende einfache Pfadkombination
        $.writeln("Warning: Using fallback path handling");
        return new Folder(basePath + "/" + inputPath.replace(/^\//, ''));
    } finally {
        // Sicherstellen, dass die Logdatei geschlossen wird
        if (logFile && logFile.exists) {
            try {
                logFile.close();
            } catch(e) {
                $.writeln("Warning: Could not close log file: " + e);
            }
        }
    }
}
